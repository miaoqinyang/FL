...
class RNN_Shakespeare(nn.Module):
    def __init__(self, vocab_size=80, embedding_dim=8, hidden_size=256):
        """Creates a RNN models using LSTM layers for Shakespeare language models (next character prediction task).
        Args:
...
class LSTMModel(nn.Module):
    def __init__(self,
        ...
        """Creates a RNN models using LSTM layers providing embedding_weights to pretrain
        Args:
            ...
            using_pretrained (bool, optional): if use embedding vector to pretrain models, set `True`, defaults to `False`
            embedding_weights (torch.Tensor, optional): vectors to pretrain models, defaults to `None`
            bid (bool, optional): if use bidirectional LSTM models, set `True`, defaults to `False`
        Returns:
            A `torch.nn.Module`.
        """
